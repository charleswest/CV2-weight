...
def qnumb(grp,img,xd):      #   what number does this group encode??
  ...
  #     compute  min and max y values for group           
        if by < miny:
            miny = by
        if by + bh > maxy:
            maxy = by + bh
        # compute x y w h for draw blue/black rectangle
    zbx,zby,zbw,zbh = cv2.boundingRect(grp[0])
    xr =zbx           #    first x value
    zbx,zby,zbw,zbh = cv2.boundingRect(grp[-1])
    wr = zbx +zbw  -  xr    #    last largest x value - first x
    
    tl = (xr-10,miny-10) ; br = ( xr +  wr+10, maxy+10)       #  10 for more room
    cv2.rectangle(img,(tl),(br),(0,0,0),5)                    #  around digit
    ...
    tpat = qptrn(ptrn,tl,br)
    return tpat


def qptrn(p,tl,br):   #   this is the pattern of a single digit
    ...
    if p in ptrn:
        ofile = r'C:\Train\{}{}{}.png'.format(tfile,xtyp,ptrn[p])
        print 'writing {} {} to file {}'.format(xtyp, ptrn[p], ofile)
        y = tl[1]; y1= br[1]; x=tl[0]; x1=br[0]   #  from above rectangle top left 
        imTx = img[ y:y1,x:x1 ].copy()             #  to bottom right
        cvs(1,imTx,'imtx')
        #cv2.imwrite(ofile,imTx)
